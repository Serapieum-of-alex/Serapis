...
for i in range(len(Qgauges)):
    ...
    st1 = Calib.annual_max_obs_q.loc[:, Sub][
        Calib.annual_max_obs_q.loc[:, Sub] != NoValue
    ].index[0]
    st2 = Calib.annual_max_hm_q.loc[:, Sub][
        Calib.annual_max_hm_q.loc[:, Sub] != NoValue
    ].index[0]
...
if PlotPoint:
    ...
    ax[0, 0].plot(
        Calib.RPObs.loc[fromd:tod, SubID].tolist(),
        Calib.annual_max_obs_q.loc[fromd:tod, SubID].tolist(),
        "o",
        zorder=6,
        color="black",
    ...
    ax[0, 0].plot(
        Calib.RPHM.loc[fromd:tod, SubID].tolist(),
        Calib.annual_max_hm_q.loc[fromd:tod, SubID].tolist(),
        "x",
        zorder=4,
        color="black",
        markersize=markersize,
...
ax[0, 2].legend(bbox_to_anchor=(0.03, 0.95), loc="upper left", borderaxespad=0.0)
# fig.tight_layout() #,pad=1.5
plt.subplots_adjust(
    hspace=0.35, wspace=0.25, top=0.95, bottom=0.07, left=0.07, right=0.95
)
